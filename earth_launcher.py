#!/usr/bin/env python3
"""
Earth Launcher - Modern Game Launcher for Raspberry Pi
...
class ModernGameLauncher:
    def __init__(self):
        ...
        # Data storage
        self.games_file = Path.home() / ".earth_launcher" / "games.json"
        self.games_file.parent.mkdir(exist_ok=True)
    ...
    def setup_installed_games_tab(self, parent):
        ...
        # Configure Treeview colors (Linux compatible)
        self.installed_tree.configure(background=self.colors['bg'],
                                    foreground=self.colors['fg'],
    ...
    def setup_repository_games_tab(self, parent):
        ...
        # Configure Treeview colors (Linux compatible)
        self.repo_tree.configure(background=self.colors['bg'],
                               foreground=self.colors['fg'],
                               fieldbackground=self.colors['bg'],
                               selectbackground=self.colors['accent'])
    ...
    def install_selected_game(self):
        ...
        def download_and_install():
            try:
                ...
                # Find the executable - look for Linux executables
                executable_path = None
                possible_executables = [
                    game_dir / game_info['executable'],
                    game_dir / f"{game_name.lower().replace(' ', '_')}",
                ...
                # Also search for any executable files in the extracted directory
                ...
                if not executable_path:
                    ...
                    self.root.after(0, lambda: self.handle_installation_error(
                        f"No executable found in {game_name}.zip. Please check the ZIP file contains a Linux executable.", 
                        progress_dialog
                    ))
                    return
    ...
    def download_game_zip(self, url, game_name, progress_dialog):
        ...
        try:
            ...
            try:
                ...
                elif game_name == "Pi Terminal Games":
                    game_code = self.create_terminal_games()
                else:
...
## How to run:
python3 {game_name.lower().replace(' ', '_')}.py
...
"""
    ...
    def launch_selected_game(self, event=None):
        ...
        if game_name in self.games:
            ...
            def launch():
                try:
                    # Check if it's a Python file and run with python3
                    if game_path.endswith('.py'):
                        subprocess.Popen(['python3', game_path], cwd=os.path.dirname(game_path))
                    else:
...
'''
    ...
    def create_terminal_games(self):
        """Create a collection of terminal-based games for Pi"""
        return '''#!/usr/bin/env python3
import random
import time
import os
...
def word_guess():
    ...
    words = ['python', 'raspberry', 'linux', 'terminal', 'game', 'computer']
